...
public class Algorithm5 extends Algorithm {
	...
	public Algorithm5(HashSet<Rectangle> rechthoeken) {
		super(rechthoeken);
	}
	...
	public HashSet<Coordinate> run(){
		...
		//Start algorimte
		RedBlackBST<Double, Rectangle> active = new RedBlackBST<>();
		while(!queue.isEmpty()){
			...
			//Als de rechthoek wel al actief is
				...
				active.delete(struct.getRechthoek().getLo().getY());
				active.delete(struct.getRechthoek().getRb().getY(), struct.getRechthoek());
			}
		}
